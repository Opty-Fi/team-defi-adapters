                    ...
                    case "approveToken(address)": {
                        ...
                        if (tokens) {
                            if (action.expect === "success") {
                                ...
                                await expect(
                                    registryContract[action.action](tokens)
                    ...
                    case "approveCreditPool(address)": {
                        ...
                        if (lqs) {
                            if (action.expect === "success") {
                                ...
                                await expect(
                                    registryContract[action.action](lqs)
                                ).to.be.revertedWith(action.message);
                            }
                        }
                        assert.isDefined(
                    ...
                    case "rateCreditPools((address,uint8)[])": {
                        ...
                        if (lqRate) {
                            if (action.expect === "success") {
                                ...
                                await expect(
                                    registryContract[action.action](lqRate)
                                ).to.be.revertedWith(action.message);
                            }
                        }
                        assert.isDefined(
                    ...
                    case "setLiquidityPoolsToAdapters((address,address)[])": {
                        ...
                        if (lqs) {
                            ...
                            for (let i = 0; i < lqs.length; i++) {
                                args.push([
                                    lqs[i].liquidityPool,
                                    adapters[lqs[i].adapterName].address,
                                ]);
                            }
                            if (action.expect === "success") {
                                ...
                                await expect(
                                    registryContract[action.action](args)
                                ).to.be.revertedWith(action.message);
                            }
                        }
                        assert.isDefined(
                    ...
                    case "setTokensHashToTokens(address[])": {
                        ...
                        if (tokensHash) {
                            if (action.expect === "success") {
                                ...
                                await expect(
                                    registryContract[action.action](tokensHash)
                                ).to.be.revertedWith(action.message);
                            }
                        }
                        assert.isDefined(
