...
import { TypedAdapterStrategies,TypedDefiPools,TypedTokens,TypedBtcTokens } from "../../../helpers/data";
import { deployAdapter, deployAdapterPrerequisites, deployAdapters, deployEssentialContracts } from "../../../helpers/contracts-deployments";
...
describe("CurveSwapPoolAdapter", () => {
  const ADAPTER_NAME = "CurveSwapPoolAdapter";
...
describe(`${testDeFiAdapterScenario.title} - CurveSwapPoolAdapter`, () => {
  ...
  for (const adapterName of adapterNames) {
    ...
    if (adapterName == "CurveSwapPoolAdapter") {
      ...
      for (const pool of pools) {
        ...
        // TODO: Refactor fundWalletToken() to get USDK,LINKUSD,SBTC, HBTC, 3Crv
        if (
                  ...
                  case "fundTestDeFiAdapterContract": {
                    ...
                    if (underlyingBalance.lt(defaultFundAmount)) {
                      try {
                        ...
                        console.log(
                          `${pool} - ${story.description} ${ethers.utils.formatUnits(defaultFundAmount, decimals)}`,
                          error,
                        );
                      }
                    }
