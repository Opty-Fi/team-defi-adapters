          ...
          for (let i = 0; i < testDeFiAdaptersScenario.stories.length; i++) {
            it(`${pool} - ${testDeFiAdaptersScenario.stories[i].description}`, async function () {
              ...
              if (getLPERC20Code !== "0x" && getERC20Code !== "0x" && getLPCode !== "0x" && +poolValue > 0) {
                ...
                const rewardTokenAddress = await adapter.getRewardToken(ZERO_ADDRESS);
                ...
                for (const action of story.getActions) {
                  switch (action.action) {
                    ...
                    case "getPoolValue(address,address)": {
                      const _poolValue = await adapter[action.action](liquidityPool, ZERO_ADDRESS);
                    ...
                    case "getLiquidityPoolToken(address,address)": {
                      const _liquidityPool = await adapter[action.action](ZERO_ADDRESS, liquidityPool);
                    ...
                    case "getUnderlyingTokens(address,address)": {
                      const _underlyingAddressFromAdapter = await adapter[action.action](liquidityPool, ZERO_ADDRESS);
          ...
          for (const action of story.setActions) {
            switch (action.action) {
              case "canStake(address)": {
                const _canStake = await adapter[action.action](ZERO_ADDRESS);
          ...
          for (const action of story.getActions) {
            switch (action.action) {
              case "getRewardToken(address)": {
                const _rewardTokenAddress = await adapter[action.action](ZERO_ADDRESS);
