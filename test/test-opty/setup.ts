...
import { ESSENTIAL_CONTRACTS as ESSENTIAL_CONTRACTS_DATA, TOKENS, ADAPTER, RISK_PROFILES } from "./utils/constants";
...
export async function deployRegistry(owner: Signer): Promise<Contract> {
  const RegistryFactory = await ethers.getContractFactory(ESSENTIAL_CONTRACTS_DATA.REGISTRY);
  let registry = await RegistryFactory.connect(owner).deploy();

  const RegistryProxyFactory = await ethers.getContractFactory(ESSENTIAL_CONTRACTS_DATA.REGISTRY_PROXY);
  ...
  registry = await ethers.getContractAt(ESSENTIAL_CONTRACTS_DATA.REGISTRY, registryProxy.address, owner);
  return registry;
}
...
async function deployEssentialContracts(owner: Signer): Promise<ESSENTIAL_CONTRACTS> {
  ...
  const StrategyProvider = await ethers.getContractFactory(ESSENTIAL_CONTRACTS_DATA.STRATEGY_PROVIDER);
  const strategyProvider = await StrategyProvider.connect(owner).deploy(registry.address);

  const HarvestCodeProvider = await ethers.getContractFactory(ESSENTIAL_CONTRACTS_DATA.HARVEST_CODE_PROVIDER);
  const harvestCodeProvider = await HarvestCodeProvider.connect(owner).deploy(registry.address);

  const RiskManagerFactory = await ethers.getContractFactory(ESSENTIAL_CONTRACTS_DATA.RISK_MANAGER);
  let riskManager = await RiskManagerFactory.connect(owner).deploy(registry.address);

  const RiskManagerProxyFactory = await ethers.getContractFactory(ESSENTIAL_CONTRACTS_DATA.RISK_MANAGER_PROXY);
  const riskManagerProxy = await RiskManagerProxyFactory.connect(owner).deploy(registry.address);
  ...
  riskManager = await ethers.getContractAt(ESSENTIAL_CONTRACTS_DATA.RISK_MANAGER, riskManagerProxy.address, owner);

  await riskManager.initialize(strategyProvider.address);

  const StrategyManager = await ethers.getContractFactory(ESSENTIAL_CONTRACTS_DATA.STRATEGY_MANAGER);
  const strategyManager = await StrategyManager.connect(owner).deploy(registry.address, harvestCodeProvider.address);
...
export async function deployVault(
  ...
  const VAULTProxyFactory = await ethers.getContractFactory(ESSENTIAL_CONTRACTS_DATA.VAULT_PROXY);
...
async function approveTokens(registryContract: Contract) {
  ...
  try {
    ...
    await registryContract.setMultipleTokensHashToTokens(tokenAddresses.map(addr => [addr]));
  } catch (error) {
