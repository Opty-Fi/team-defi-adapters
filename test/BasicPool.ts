...
import OptyAaveBorrowPoolProxy from "../build/AaveBorrowPoolProxy.json";
import OptyCurveDepositPoolProxy from "../build/CurveDepositPoolProxy.json";
import OptyCreamDepositPoolProxy from "../build/CreamDepositPoolProxy.json";
import OptyDForceDepositPoolProxy from "../build/DForceDepositPoolProxy.json";
import OptyFulcrumDepositPoolProxy from "../build/FulcrumDepositPoolProxy.json";
import OptyHarvestDepositPoolProxy from "../build/HarvestDepositPoolProxy.json";
import OptyYearnDepositPoolProxy from "../build/YearnDepositPoolProxy.json";
...
describe("OptyTokenBasicPool for DAI", async () => {
    ...
    before(async () => {
        ...
        optyStrategy = await deployContract(wallet, OptyStrategy, [
        ...
        optyCompoundDepositPoolProxy = await deployContract(
            wallet,
            OptyCompoundDepositPoolProxy
        );
        console.log("Compound Pool Proxy: ", optyCompoundDepositPoolProxy.address);
        assert.isDefined(
            optyCompoundDepositPoolProxy,
            "OptyCompoundDepositPoolProxy contract not deployed"
        );

        optyAaveDepositPoolProxy = await deployContract(
            wallet,
            OptyAaveDepositPoolProxy
        );
        console.log("Aave Pool Proxy: ", optyAaveDepositPoolProxy.address);
        assert.isDefined(
            optyAaveDepositPoolProxy,
            "OptyAaveDepositPoolProxy contract not deployed"
        );
                ...
                if (protocol == "compound") {
                    it(
                        ...
                        async () => {
                            ...
                            tempArr.push(
                                strategies.compound.pool,
                                strategies.compound.outputToken,
                                strategies.compound.isBorrow
                            );
                    ...
                    it(
                        ...
                        async () => {
                            await optyRegistry.setLiquidityPoolToDepositPoolProxy(
                                strategies.aave.pool,
                                optyAaveDepositPoolProxy.address
                            );
                            // strategies.aave[4] = optyAaveDepositPoolProxy.address.toString()
                            let tempArr: (string | boolean)[] = [];
                            tempArr.push(
                                strategies.aave.pool,
                                strategies.aave.outputToken,
                                strategies.aave.isBorrow
                            );
                            strategySteps = [tempArr];
    ...
    async function testUserDepositRebalance(strategySteps: (string | boolean)[][]) {
        ...
        prevUserOptyTokenBalanceWei = await optyTokenBasicPool.balanceOf(
            wallet.address
        );
