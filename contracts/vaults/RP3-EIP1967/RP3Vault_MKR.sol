 ...
 *
 * @dev Opty.Fi's RP3 Vault contract for MKR token
 */
contract RP3Vault_MKR is VersionedInitializable, IVault, ERC20, Modifiers, ReentrancyGuard, VaultStorage, Deployer {
    ...
    constructor(
        ...
        public
        ERC20(
            string(abi.encodePacked("op ", "Maker", " RP3", " vault")),
            string(abi.encodePacked("op", "MKR", "RP3Vault"))
        )
        Modifiers(_registry)
    {
    ...
    function initialize(
        ...
        _setName(string(abi.encodePacked("op ", "Maker", " RP3", " vault")));
        _setSymbol(string(abi.encodePacked("op", "MKR", "RP3Vault")));
