 ...
 *
 * @dev Opty.Fi's RP1 Vault contract for underlying tokens (for example DAI)
 */
contract RP1Vault is VersionedInitializable, IVault, ERC20, Modifiers, ReentrancyGuard, VaultStorage, Deployer {
    ...
    constructor(
        ...
        public
        ERC20(
            string(abi.encodePacked("op ", ERC20(_underlyingToken).name(), " RP1", " vault")),
            string(abi.encodePacked("op", ERC20(_underlyingToken).symbol(), "RP1Vault"))
        )
        Modifiers(_registry)
    {
    ...
    function initialize(
        ...
        _setName(string(abi.encodePacked("op ", ERC20(_underlyingToken).name(), " RP1", " vault")));
        _setSymbol(string(abi.encodePacked("op", ERC20(_underlyingToken).symbol(), "RP1Vault")));
