 ...
 *
 * @dev Opty.Fi's RP2 Pool contract for underlying tokens (for example DAI)
 */
contract RP2Vault is VersionedInitializable, IVault, ERC20, Modifiers, ReentrancyGuard, PoolStorage, Deployer {
    ...
    constructor(
        ...
        public
        ERC20(
            string(abi.encodePacked("op ", ERC20(_underlyingToken).name(), " RP2", " pool")),
            string(abi.encodePacked("op", ERC20(_underlyingToken).symbol(), "RP2Pool"))
        )
        Modifiers(_registry)
    {
