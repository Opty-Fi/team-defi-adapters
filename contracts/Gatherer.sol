    ...
    function getHarvestCodes(address _optyPool, address _rewardToken, address _underlyingToken, uint _rewardTokenAmount) public view returns(bytes[] memory _codes) {
        if (_rewardTokenAmount > 0) {
            ...
            if(_amounts[_path.length - 1] > 0) {
                    ...
                    _codes[2] = abi.encode(router,abi.encodeWithSignature("swapExactTokensForTokens(uint256,uint256,address[],address,uint256)",_rewardTokenAmount,uint(0),_path,_optyPool,uint(-1)));
            }
        }
    }
